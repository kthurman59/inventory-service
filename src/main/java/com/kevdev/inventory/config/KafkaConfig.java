...
public class KafkaConfig {
    ...
    public NewTopic ordersCreatedTopic() {
        return TopicBuilder.name("orders.created").partitions(3).replicas(1).build();
    }

    @Bean
    public NewTopic inventoryReservationResultsTopic() {
        return TopicBuilder.name("inventory.reservation.results").partitions(3).replicas(1).build();
    }
